...
enum TrackMetrics {
    ...
    /// cases — so Track and Live agree on which metric reads as "first"
    /// instead of each screen inventing its own ordering. Pulse (`.hr`) and
    /// Calm Power (`.vti`) are deliberately excluded, along with every other
    /// `LiveMetric` case Track has no chart for (`.sdnn`, `.coherence`,
